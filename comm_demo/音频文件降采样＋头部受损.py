...
class SimulationSession:
    ...
    def build_result(self) -> SimulationResult:
        restored_text, restored_image, restored_audio_samples, restored_audio_rate, restored_audio_wav = restore_output(
            self.restored_bytes or b"", self.source
        )
...
def prepare_source(kind: str, text: str, path: str) -> SourceArtifact:
    ...
    with wave.open(str(file_path), "rb") as wav_file:
        ...
        optimized_samples, optimized_rate, notes = _resample_audio(samples, original_rate)
        payload = pack_audio_data(optimized_samples, optimized_rate)
        return SourceArtifact(
            kind=kind,
            raw_bytes=payload,
...
# 对语音数据进行重采样（降采样）
MAX_AUDIO_SAMPLE_RATE = 4000
def _resample_audio(samples: np.ndarray, sample_rate: int) -> tuple[np.ndarray, int, list[str]]:
    notes: list[str] = []
    working = samples.astype(np.float32, copy=False)
    if sample_rate > MAX_AUDIO_SAMPLE_RATE and len(working) > 1:
        target_rate = MAX_AUDIO_SAMPLE_RATE
        target_len = max(1, int(round(len(working) * target_rate / sample_rate)))
        src = np.arange(len(working), dtype=np.float32)
        dst = np.linspace(0, len(working) - 1, num=target_len, dtype=np.float32)
        working = np.interp(dst, src, working).astype(np.float32, copy=False)
        notes.append(f"音频已自动降采样到 {target_rate} Hz。")
        sample_rate = target_rate
    optimized = np.clip(np.round(working), -32768, 32767).astype(np.int16)
    return optimized, sample_rate, notes
...
def restore_output(restored_bytes: bytes, source: SourceArtifact) -> tuple[str, np.ndarray | None, np.ndarray | None, int, bytes | None]:
    kind = source.kind
    if kind == "文本":
    ...
    try:
        ...
        if source.audio_samples is not None and source.sample_rate > 0:
            expected_count = len(source.audio_samples)
            payload = restored_bytes[8:] if len(restored_bytes) > 8 else restored_bytes
            if len(payload) > 0:
                quantized = np.frombuffer(payload[:expected_count], dtype=np.int8).copy()
                if len(quantized) < expected_count:
                    quantized = np.pad(quantized, (0, expected_count - len(quantized)))
                samples = (quantized.astype(np.int16, copy = False) * 256).astype(np.int16, copy = False)
                wav_bytes = pcm_to_wav_bytes(samples, source.sample_rate)
                return (
                    "语音恢复完成（头部受损，已使用原始元数据兜底）\n"
                    f"采样率: {source.sample_rate} Hz\n"
                    f"采样点数: {len(samples)}",
                    None,
